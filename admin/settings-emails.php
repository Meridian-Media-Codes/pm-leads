 ...
 * Saving handled in admin/menu.php
 */
...
function pm_leads_allowed_merge_tags($key) {
    ...
    $vendor = [
        '{vendor_name}','{vendor_email}','{vendor_company}',
        '{vendor_phone}','{vendor_service_radius}','{vendor_credits}',
    ];
    ...
    switch ($key) {
        ...
        case 'lead_purchased_customer':   return array_merge($customer, $vendor, $job_misc, $site);
...
function pm_leads_render_merge_tag_help($key) {
    ...
    foreach ($tags as $t) {
        echo '<code style="padding:4px 6px;background:#f6f7f7;border:1px solid #dcdcde;border-radius:3px;">'.esc_html($t).'</code>';
    }
...
function pm_leads_render_email_settings() {
    ...
    $tab('new_lead_customer','New Lead → Customer');
    $tab('new_lead_vendors','New Lead → Vendors');
    $tab('lead_purchased_vendor','Lead Purchased → Vendor');
    $tab('lead_purchased_customer','Lead Purchased → Customer');
    $tab('credits_purchased_vendor','Credits Purchased → Vendor');
    $tab('low_credits_vendor','Low Credits → Vendor');
    $tab('vendor_approved_vendor','Vendor Approved → Vendor');
